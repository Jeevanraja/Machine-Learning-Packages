...
def elasticlogifit(X,Y,y,eta=0.0001,epochs=1000,lamda1=10,lamda2=10):
    ...
    for i in range(epochs):
        P=softmax(X.dot(W))
        J.append(cross_entropy(Y,P)+lamda2*np.sum(W.T.dot(W))+lamda1* np.sum(np.abs(W)))
        W -= eta*(X.T.dot(P-Y)+lamda1*np.sign(W)+lamda2*W)
    P=softmax(X.dot(W))
